...
defmodule OMG.API.State.Transaction.Signed do
  ...
  def signed_hash(%__MODULE__{raw_tx: tx, sigs: sigs}) do
    ...
    hash_with_sigs = Enum.reduce(sigs, tx_hash, &<>/2)
  ...
  defp reconstruct_tx(
    ...
    with :ok <- signature_length?(sig1),
      ...
      inputs = [
        [blknum: int_parse(blknum1), currency: parsed_cur12, txindex: int_parse(txindex1), oindex: int_parse(oindex1)],
        [blknum: int_parse(blknum2), currency: parsed_cur12, txindex: int_parse(txindex2), oindex: int_parse(oindex2)]
      ]
